...
3. **quality** -- retrieval-quality regression gate: builds the release binary, runs `bench/bench.mjs --quality-only` against the committed baseline (`bench/baselines/quality.json`), and fails on a >25% relative drop in recall@10 / ndcg@10 / hits@1. It also fails outright when the baseline's `provenance.platform` differs from the runner's: the gate refuses to compare rather than demoting the result, so a baseline blessed anywhere but ubuntu-latest fails this job instead of quietly passing. Bless a new baseline via the "Regenerate quality baseline" workflow_dispatch.
