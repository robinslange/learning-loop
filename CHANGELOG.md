...
- **`lib/file-lock`: mtime-fallback stat failures now surface via `logError`.** The inner `catch {}` inside `tryRemoveIfStale`'s mtime fallback was the one genuine silent failure among four empty catches in `file-lock.mjs`. It fires when (a) the lockfile exists, (b) its PID is unreadable, and (c) `statSync` then *also* fails with something other than ENOENT — a genuinely unusual race worth surfacing. ENOENT stays silent (expected race during concurrent stale-lock recovery). The three intentional silent catches (`releaseLock`'s idempotent closeSync + unlinkSync, plus the outer `tryRemoveIfStale` fallback) gained explanatory comments. `tryRemoveIfStale` is now exported with an optional `statFn` dependency seam for testing.
