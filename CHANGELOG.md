...
- **`skill` on a provenance record is derived when the caller omitted it, and validated against the installed skill set either way (#73).** Most hook-emitted events have no notion of "skill" on their own, so `deriveSkill` fills the field from the session's current-skill marker, including onto `agent-result`: a dispatched subagent has no notion of skill either, but the session that spawned it does, and that gap alone accounted for 3,591 events with no skill attribution. Whatever ends up in `skill`, caller-supplied or derived, is checked against the `plugin/skills/*/SKILL.md` directory, accepting both the bare name and the `learning-loop:`-prefixed form; a value that does not match becomes `unknown`, logged by the rejected value's length only. Seven agent files (`note-writer.md`, `note-scorer.md`, `discovery-researcher.md` and four others) no longer hardcode a single skill literal despite being dispatched from several skills each; that literal was itself a misattribution the derivation now corrects at runtime.
