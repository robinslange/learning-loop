...
- **Sharpened `/reflect`'s dedup, routing, and capture, and cut its process-spawn cost.** Three classes of change, all behind the same green test net (`reflect-new-notes-track`, `refinement-validate`, the new `strip-reflect-sid`/`sweep-scan-vault`/`resolve-paths-sh` suites). _Learning quality:_ the duplicate-check threshold dropped from `0.90` to the project's own calibrated `0.85`/`0.74` edges (`hook-config.mjs`) — `top_match_similarity` is raw cosine between a short learning summary and a full note, so it almost never cleared `0.90` even for true dupes, and "update over create" had silently collapsed to "always create"; the route-to-store rule became a crisp behaves-vs-world test with a gated split clause (no more same-sentence double-writes, there is no cross-store dedup); auto-memory dedup is now grep-gated to read candidate bodies instead of eyeballing the index; and a forcing line makes the three categories that hide in a fast pass (mid-session corrections, project-context shifts, unstated repeated behavior) explicit without pressuring fabrication. _Correctness:_ Step 4.6.g cleanup now runs on the 0-candidate path too (a no-candidate run used to leak every note's transient `reflect_sid` stamp into the vault permanently); the `reflect_sid` strip is frontmatter-scoped so a body line starting with `reflect_sid:` survives; and the stale `reflect-track.mjs` header comment (which still described the superseded `$TMPDIR`-keyed marker the rest of the file documents as a fixed bug) was corrected. _Speed:_ a new `resolve-paths.mjs --sh` mode lets each bash fence resolve every run-invariant path from one spawn instead of two or three, and both `/reflect` python3 heredocs (the Step 4.4 candidate walk, the Step 4.6.g per-note strip) folded into node — the walk into a new `sweep-hook-replay.mjs --scan-vault` mode that carries the explicit 5-folder allowlist (never the `4-projects`-including `listVaultNotes` denylist), the strip into `strip-reflect-sid.mjs`. Step 2.5 also gains an optional up-front incremental index so dedup sees notes subagents wrote earlier in the session. A cold 5-domain adversarial review (each finding 3-skeptic-verified) confirmed zero invariant breaks and zero live bugs; its 16 findings — all test-coverage gaps and doc drift on correct code — were closed: a regression guard on the Step 4.4 fence's `--sh` resolver, a real-`shQuote` eval-safety test (apostrophe path round-trip), a string-guard on the 0-candidate 4.6.g-cleanup contract, an end-to-end `--scan-vault` CLI test plus argument-guard hardening, a parameterized all-5-folder allowlist-inclusion test, a config-pinned threshold test, and corrected handshake/attribution comments.
