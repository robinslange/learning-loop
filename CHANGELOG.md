...
- **A pasted id list no longer stalls the frontmatter repairer.** `hasUngroundedFactualSignal` runs the quantity pattern once per paragraph, and that pattern can begin a match at every digit of a number: from each start the engine walks the rest of the run before the unit fails to follow, so cost is quadratic in the length of a single digit run, not the note. A pasted id list, a phone list, a long decimal, measured 20s at 100k characters against `normalise-frontmatter.mjs`, a full-vault batch pass with no size guard anywhere in the call chain.
  ...
  The pattern is unchanged. Three rewrites of it were tried and all three were wrong: two theorised the cost was internal backtracking between overlapping character classes and changed the timing not at all, because the cost was never inside the match, it was in the number of positions the match is attempted from. The third removed the restarts with a lookbehind and was fast, but differential fuzz showed it silently stopped flagging a figure written without a leading zero (`.5%`), and every variant that fixed that either reopened the cost or lost a figure the old pattern caught mid-run. A pattern that starts anywhere is what makes it both correct and slow.

  So the input is bounded instead, and the first attempt at that was wrong too: deleting any run of 40 or more digits and separators also deleted real signals, because `9...9mg` is a quantity however long it is, and `n=`, `p<0.` and a bare `<` trigger before their digits rather than after. Boundary-targeted fuzz found both classes immediately where random fuzz had missed them for 400k cases. What ships keeps the first 39 characters of a long run and drops only the tail, and leaves a run whole when a unit follows it: every pattern needs at most a few characters after its trigger, so the tail is precisely the part that can only add restart positions. 20,862ms to 1ms at 400k characters, with zero disagreements against the unclipped text across run lengths 30 to 60 against every unit and trigger prefix, and across 500k random numeric-alphabet strings.
