...
- **The per-session fetch budget holds under parallel `/research` fetches.** The counter was a read-then-rewrite, so concurrent gateway processes lost increments and could all pass the check together. Each fetch now appends one byte and reads the count back as the file size (#91).
