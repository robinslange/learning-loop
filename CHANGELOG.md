...
- **A pasted id list no longer stalls the frontmatter repairer.** `hasUngroundedFactualSignal` runs the quantity pattern once per paragraph, and that pattern started at every digit of a number: from each start the engine walked the rest of the run before the unit failed to follow, so cost was quadratic in the length of the number, not the note. A long separator-heavy digit run, a pasted id list, a phone list, a long decimal, measured 20s at 100k characters against `normalise-frontmatter.mjs`, a full-vault batch pass with no size guard anywhere in the call chain. The fix is a lookbehind refusing a start preceded by a digit or separator, which leaves exactly one viable start per run: 20,862ms to 1ms, with no match the old pattern found and the new one misses (a quantity begins at the front of its number). The first two attempts at this rewrote the quantity pattern's internals on a theory about overlapping character classes and changed nothing measurable, because the cost was never inside the match, it was in the number of places the match was attempted.
