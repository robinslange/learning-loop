...
1. **Search the vault** for related notes:
   ...
   - Use `ll-run vault-search.mjs search "<topic>" --rerank` for semantic matches beyond keyword search
   - Once a strong match is found, use `ll-run vault-search.mjs similar "<best-match-note>"` to find semantically related notes
...
3. Run: `ll-run vault-search.mjs discriminate <paths>`
