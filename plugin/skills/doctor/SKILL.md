...
| `shims-exist` (missing or not executable)                          | `node ${CLAUDE_PLUGIN_ROOT}/scripts/install-shims.mjs --install`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `shims-exist` (present, no resolved root ships `scripts/shim.mjs`) | Upgrade the plugin. Do NOT run `install-shims.mjs` here: the shims are already correct, and rewriting them leaves them resolving to the same tree and exiting 1 again. Until the upgrade lands, call scripts directly with `node ${CLAUDE_PLUGIN_ROOT}/scripts/<script>.mjs`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `duplicate-gate-health` (repeated timeouts)                        | `node ${CLAUDE_PLUGIN_ROOT}/scripts/watch.mjs` (start the warm daemon so the duplicate gate uses the socket instead of cold-starting the model on every write)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `duplicate-gate-health` (stale daemon)                             | Kill the running ll-watch process then `node ${CLAUDE_PLUGIN_ROOT}/scripts/watch.mjs` (restarts with the new binary that supports duplicate-scan)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `watch-daemon-status` (stale pidfile)                              | `rm <pidfile>` then `node ${CLAUDE_PLUGIN_ROOT}/scripts/watch.mjs` (no arguments — that is the background-start invocation; `start` is not a subcommand)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **`hub holds: nothing`** — as of that cycle the hub held no index for this
  ...
  is degraded — check its `/health`. _This is the signature of the 2026-07
...
```bash
PLUGIN_DATA="$(node ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-paths.mjs PLUGIN_DATA)"
echo "$PLUGIN_DATA"
```
