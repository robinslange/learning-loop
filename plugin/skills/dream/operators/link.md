...
```markdown
### LINK
- `feedback_graphite_workflow.md` <-> `project_acme.md` (shared: Graphite CLI workflow)
```

Emit provenance after each operation: `node "${CLAUDE_PLUGIN_ROOT}/scripts/provenance-emit.js" '{"agent":"dream","skill":"dream","action":"link","target":"FILENAME"}'`
