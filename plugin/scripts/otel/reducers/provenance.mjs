...
import { readRecords, monthlyFiles, countBy } from '../reduce.mjs';
...
export function reduceProvenance({ pluginData, timeUnixMs }) {
  ...
  return [
    ...countBy(known, { name: 'provenance_actions', stream: STREAM, by: ['action'], timeUnixMs }),
    ...countBy(known, { name: 'provenance_agent', stream: STREAM, by: ['agent'], timeUnixMs }),
    ...countBy(known, { name: 'provenance_skill', stream: STREAM, by: ['skill'], timeUnixMs }),
    ...countBy(known, { name: 'provenance_folder', stream: STREAM, by: ['folder'], timeUnixMs }),
  ];
}
