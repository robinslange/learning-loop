...
Reindexing is continuous, not post-session. `hooks/session-start/watch-daemon.mjs` spawns `ll-search watch` at SessionStart and supervises it via the per-vault pidfile at `<vault>/.vault-search/watch.pid`. The watcher is a long-running process that incrementally reindexes notes as they change (fs-watch-driven). It also hosts the UDS daemon for duplicate-scan requests over a unix domain socket. On binary upgrade, mtime changes trigger SIGTERM + respawn. The federation config is not carried across the process's life: `sync/watch.rs::reload_config` re-reads `config.json` on every federation tick, so a config written by `join` or `link request` under a live daemon takes effect within one sync interval without a restart, and a file that stops parsing keeps the last one that did rather than disabling federation. A one-shot migration on first run after an upgrade reaps any legacy daemon still holding the old `$CLAUDE_PLUGIN_DATA/watch.pid` path. The Stop hook (`hooks/stop-nudge.js`) does not reindex; it only emits reflect/dream nudges based on transcript size and memory-file delta.
...
Every `hooks.json` command is `node "${CLAUDE_PLUGIN_ROOT}/hooks/run.mjs" hooks/<handler>.js`. Claude Code pins a session to the version directory it loaded; `run.mjs` asks `scripts/lib/plugin-meta.mjs` `activeRoot()` for the install recorded in `~/.claude/plugins/installed_plugins.json` and runs the handler from there when that install is a sibling in the same cache and still ships the handler. A release therefore reaches open sessions on their next hook call. `cache-cleanup` no longer deletes superseded version directories; Claude Code marks them `.orphaned_at` and reaps them. `ll-run`, `ll-paths` and `ll-watch` follow the same rule through `scripts/lib/shims.mjs` (text) and `scripts/shim.mjs` (behaviour); `ll-search` is a node-free shell shim over plugin-data. SessionStart rewrites any shim whose text changed.
