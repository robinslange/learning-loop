...
Reindexing is continuous, not post-session. `hooks/session-start/watch-daemon.mjs` spawns `ll-search watch` at SessionStart and supervises it via the per-vault pidfile at `<vault>/.vault-search/watch.pid`. The watcher is a long-running process that incrementally reindexes notes as they change (fs-watch-driven). It also hosts the UDS daemon for duplicate-scan requests over a unix domain socket. On binary upgrade, mtime changes trigger SIGTERM + respawn — and only that: the federation config it syncs against is read once in `sync/watch.rs::run_watch_async` and held for the process's life, so a `config.json` written by `join` or `link request` under a live daemon needs `ll-watch stop && ll-watch` to take effect there. A one-shot migration on first run after an upgrade reaps any legacy daemon still holding the old `$CLAUDE_PLUGIN_DATA/watch.pid` path. The Stop hook (`hooks/stop-nudge.js`) does not reindex; it only emits reflect/dream nudges based on transcript size and memory-file delta.
