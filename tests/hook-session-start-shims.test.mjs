...
// The trigger that predates the text comparison, and the one case the text
// comparison cannot express on its own: ll-run was added to SHIM_NAMES after
// installs existed, so those installs sit at the current version holding a
// complete-looking set that is missing a shim they have never received.
test('session start installs a shim the install has never received', () => {
  withShims(({ home, bin }) => {
    const absent = join(bin, shimFileName('ll-run'));
    rmSync(absent, { force: true });
    runCacheCleanup(home);
    assert.equal(readFileSync(absent, 'utf8'), renderShim('ll-run'));
  });
});
