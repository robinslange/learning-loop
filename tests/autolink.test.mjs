...
test('autolink: Write with [[wikilink]] appends backlink to target exactly once, frontmatter intact', () => {
  const vault = makeVault();
  const stdin = writeStdin(vault, '0-inbox/new-note.md', '---\ntags: [test]\n---\n\nLinks to [[sleep]].\n');
...
test('autolink: re-running the hook does not duplicate the backlink (idempotent)', () => {
  const vault = makeVault();
...
test('autolink: target without trailing newline gets backlink on a fresh line, not glued', () => {
  const vault = makeVault();
...
test('autolink: wikilink to a note that exists nowhere is a no-op, no errors', () => {
  const vault = makeVault();
  const stdin = writeStdin(vault, '0-inbox/new-note.md', 'See [[does-not-exist]] and [[sleep]].\n');
  const r = run(stdin, vault, (pd) => stubBinary(pd));
  try {
    assert.equal(r.exitCode, 0);
...
test('autolink: snapshot entry whose file is gone (ENOENT) is pruned and the loop continues', () => {
  const vault = makeVault();
...
test('autolink: unwritable target — hook exits 0, error isolated to hook-errors log, target uncorrupted', () => {
  const vault = makeVault();
  ...
  try {
    ...
    assert.equal(readFileSync(targetPath, 'utf8'), SLEEP_NOTE, 'read-only target must be untouched');
    const errors = readAutolinkErrors(r.pluginDataDir);
    assert.equal(errors.length, 1, `expected one isolated runAutolink error: ${JSON.stringify(errors)}`);
  } finally {
...
test('autolink: failed tool_response short-circuits — no vault mutation', () => {
  const vault = makeVault();
...
test('autolink: writes under _system/ are not vault notes — no backlinks', () => {
  const vault = makeVault();
...
test('autolink similarity: appends at most 3 above-threshold candidates, skips low scores and existing links, writes reciprocal backlinks', () => {
  const vault = makeVault();
  ...
  try {
    ...
    assert.equal(source.split('[[circadian]]').length - 1, 1, 'existing link must not be duplicated');
...
test('autolink: Edit tool appends backlinks from disk content but never runs the similarity path', () => {
  const vault = makeVault();
  mkdirSync(join(vault, '.vault-search'), { recursive: true });
  writeFileSync(join(vault, '.vault-search', 'vault-index.db'), '');
  ...
  const r = run(
    ...
    vault,
    (pd) => stubBinary(pd, { similarJson: '[{"path":"3-permanent/circadian.md","score":0.9}]', markerPath }),
  );
  try {
    assert.equal(r.exitCode, 0);
    assert.equal(readFileSync(join(vault, '3-permanent', 'sleep.md'), 'utf8'), SLEEP_NOTE + '[[edited-note]]\n');
    assert.ok(!existsSync(markerPath), 'similarity subcommand must not run for Edit');
    assert.equal(readFileSync(filePath, 'utf8'), 'Now links [[sleep]].\n', 'no auto-links appended on Edit');
  } finally {
    r.cleanup();
    rmSync(vault, { recursive: true, force: true });
  }
...
test('autolink: similarity binary hanging past its timeout is contained — backlinks still applied, no module failure', () => {
  const vault = makeVault();
  mkdirSync(join(vault, '.vault-search'), { recursive: true });
  writeFileSync(join(vault, '.vault-search', 'vault-index.db'), '');
  ...
  const r = run(stdin, vault, (pd) =>
    stubBinary(pd, { similarJson: '[{"path":"3-permanent/circadian.md","score":0.9}]', sleepSecs: 3 }),
  );
  try {
    assert.equal(r.exitCode, 0, `stderr: ${r.stderr}`);
    // Wikilink backlink phase ran before the similarity call.
    assert.equal(readFileSync(join(vault, '3-permanent', 'sleep.md'), 'utf8'), SLEEP_NOTE + '[[new-note]]\n');
    ...
    assert.equal(readFileSync(join(vault, '3-permanent', 'circadian.md'), 'utf8'), CIRCADIAN_NOTE);
