...
describe('review-shadow', () => {
    ...
    for (let i = 0; i < 30; i++) {
      entries.push(
        JSON.stringify({
          ...
          backends: {
            vault: { latency_ms: 100 + i, hits: 5 },
            episodic: { latency_ms: 900, raced_out: false },
          },
    ...
    for (let i = 0; i < 90; i++) {
      entries.push(
        JSON.stringify({
          ts: now.toISOString(),
          session_id: 's1',
          prompt: `non-matching question number ${i}`,
          gate: { passed: false, vault_top_score: 0.1, episodic_top_score: 0.05, threshold: 0.35 },
          backends: {
            vault: { latency_ms: 120, hits: 0 },
            episodic: { latency_ms: 800, hits: 0, raced_out: false },
          },
        }),
      );
    }
  ...
  it('fires INFRASTRUCTURE verdict when backends are unhealthy', () => {
    ...
    const logPath = join(
      subdir,
      `shadow-injection-${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}.jsonl`,
    );
    const entries = [];
    for (let i = 0; i < 100; i++) {
      entries.push(
        JSON.stringify({
          ...
          backends: {
            ...
            episodic: {
              latency_ms: 0,
              hits: 0,
              error: 'spawn episodic-memory ENOENT',
              raced_out: false,
            },
          },
        }),
