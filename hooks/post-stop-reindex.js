...
const trace = (msg) => {
  if (DEBUG) process.stderr.write(`[reindex] ${msg}\n`);
};
...
if (!binary) {
  trace('exit: no binary');
  process.exit(0);
}

const dbPath = join(vaultRoot, '.vault-search', 'vault-index.db');
if (!existsSync(dbPath)) {
  trace(`exit: no db at ${dbPath}`);
  process.exit(0);
}

if (isLockHeld()) {
  trace('exit: lock held');
  process.exit(0);
}
