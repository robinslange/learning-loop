...
const pluginVersion = (() => {
  try {
    return JSON.parse(readFileSync(join(PLUGIN_DIR, "package.json"), "utf-8"))
      .version;
  } catch {
    return "0.0.0";
  }
...
if (updateCacheFile) {
  ...
  try {
    ...
    if (cached.checked && Date.now() / 1000 - cached.checked < 3600)
      shouldCheck = false;
  } catch {}
...
try {
  ...
  if (depOutput && depOutput !== "{}") {
    ...
    const issues = Object.entries(deps).filter(
      ([, v]) => v.status !== "installed",
    );
    if (issues.length > 0) {
...
try {
  ...
  const activeConfig =
    pluginDataConfig && existsSync(pluginDataConfig)
      ? pluginDataConfig
      : legacyConfig;
  ...
  const configNewer =
    configExists &&
    markerExists &&
    statSync(activeConfig).mtimeMs > statSync(configMarker).mtimeMs;
...
function isWatchRunning() {
  ...
  try {
    const pid = parseInt(
      readFileSync(join(pluginData, "watch.pid"), "utf8").trim(),
      10,
    );
    process.kill(pid, 0);
    return true;
...
if (depsAllSatisfied) {
  ...
  context +=
    "2. (Skipped — episodic memory plugin not installed. Run /init to set up.)\n";
}
...
context +=
  "6. When corrected, immediately save to auto-memory as feedback. No delay.\n";
context +=
  "7. After substantial work, suggest /reflect to consolidate learnings.\n";
context +=
  "Keep retrieval lightweight — one line per insight, not a wall of text.\n";
  ...
  try {
    ...
    try {
      ...
      appendFileSync(
        join(
          retrievalDir,
          `access-${new Date().toISOString().slice(0, 7)}.jsonl`,
        ),
        entry + "\n",
      );
    } catch {}
