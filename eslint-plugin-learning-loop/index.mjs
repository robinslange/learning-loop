...
// Seven rules. `no-raw-lockfile`, `no-url-pathname`,
// `no-handwritten-trust-envelope`, `no-raw-telemetry-append` and
// `no-empty-catch` run at "error"; `no-process-env-outside-env-module` and
// `no-direct-jsonparse` are still "off" pending consumer migration, each
// currently fails on shipped code. See eslint.config.mjs for the live
// settings and ARCHITECTURE.md ("critical invariants") for what that means
// for the invariants they back.
...
import noRawTelemetryAppend from './rules/no-raw-telemetry-append.mjs';
...
export default {
  ...
  rules: {
    ...
    'no-raw-telemetry-append': noRawTelemetryAppend,
  },
};
