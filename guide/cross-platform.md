...
- **CLI shims are `.cmd` files.** `/init` writes `ll-watch.cmd`, `ll-search.cmd` and `ll-paths.cmd` to `%USERPROFILE%\.local\bin\`. cmd.exe does not add this directory to `PATH` automatically. After install, run once in cmd.exe: `setx PATH "%USERPROFILE%\.local\bin;%PATH%"` then restart your terminal. PowerShell `.ps1` shims are not currently generated — the `.cmd` files work in both cmd.exe and PowerShell.
...
Run the cross-platform smoke test against your install. Both commands use the shims that `/init` installs into `~/.local/bin` — they work from any terminal (`CLAUDE_PLUGIN_DATA` is only set inside hook and skill processes, so don't rely on it in your shell):
