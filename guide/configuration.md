...
| PostToolUse (Write\|Edit\|Task\|Skill\|Agent) | post-tool.js            | Coalesced dispatcher. Loads one vault snapshot, then runs the provenance, reflect-track, autolink, and edge-infer modules in fixed order (cheap load-bearing modules first, so a hook timeout only drops enrichment) with per-module failure isolation. Non-write tool events only run provenance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
One asymmetry worth watching once real usage starts: the post-tool chain runs once per file in a patch, and its only deadline is the 12s `hooks.json` SIGKILL. A patch touching many vault notes spends that budget once per note, where a Claude Code write can only ever cost one file's worth. Autolink's one slow call is bounded by its own `execFileSync` timeout; nothing else inside the chain is.
...
| `LL_RERANKER_MODEL_PATH`, `LL_RERANKER_TOKENIZER_PATH` | Build-time only: point `ll-core`'s build script at a local reranker model and tokenizer instead of downloading them                                                                                                                                            |
