...
Whatever ends up in `skill`, caller-supplied or derived, is validated against the installed skill directory (`plugin/skills/*/SKILL.md`), accepting both the bare name (`reflect`) and the `learning-loop:`-prefixed form, and normalising to the bare name. A value that does not match an installed skill becomes `unknown`, and the rejection is logged by the length of the rejected value only, never its contents.
