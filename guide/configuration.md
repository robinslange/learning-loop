...
These hooks are the core of the plugin's value. Without them, the agent can skip verification, promote unsourced notes, and write in its default voice. With them, these failures are structurally impossible.
...
| `WebSearch` and `WebFetch` are hosted tools that never reach the local hook path                  | `web-guard.js` cannot see them. The Codex-only hooks file wires it to `Bash` instead, where it denies a shell fetcher aimed at a remote URL. Hosted search stays ungoverned — it is instruction-level, not enforced |
