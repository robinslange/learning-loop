...
model: haiku
tools: Read, Grep, Glob, Bash, mcp__plugin_episodic-memory_episodic-memory__search
---
...
1. **Search the vault** for related notes:
   ...
   - Use `node ${CLAUDE_PLUGIN_ROOT}/scripts/vault-search.mjs search "<topic>" --rerank` for semantic matches beyond keyword search
   - Once a strong match is found, use `node ${CLAUDE_PLUGIN_ROOT}/scripts/vault-search.mjs similar "<best-match-note>"` to find semantically related notes
